...
import { getStudentDashboardData, enrichStudent } from "@/lib/selectors"

export function StudentDashboard() {
  const { user } = useAuth()

  const { data, loading } = usePageData((d) => {
    const raw = getStudentDashboardData(d, user?.id)
    if (!raw) return null
    return { ...raw, student: enrichStudent(d, raw.student) }
  })
  ...
  const promotionLabel = (student as any).promotionName && (student as any).promotionName !== "—"
    ? (student as any).promotionName
    : ""

  return (
    <DashboardLayout
      title={`${i18n.common.greeting}, ${student.user?.first_name || student.first_name || ""}`}
      subtitle={[student.matricule, promotionLabel, student.phone_number].filter(Boolean).join(" · ")}
      stats={
        <>
          <KPICard
