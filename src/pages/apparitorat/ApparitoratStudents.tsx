...
import { matchesStudentStatusFilter } from "@/lib/studentStatus";
...
export function ApparitoratStudents() {
  ...
  const resolveAcademicYearId = (s: StudentRow) => {
    if (s.academic_year_id) return s.academic_year_id;
    if ((s as any).academic_year?.id) return (s as any).academic_year.id;

    const histories = ((s as any).histories ||
      (s as any).academic_histories ||
      []) as any[];
    const current =
      histories.find(
        (h) => (h?.status || h?.state || h?.statut) === "en_cours",
      ) || histories[0];
    return current?.academic_year_id || current?.academic_year?.id || "";
  };

  const resolveAcademicYearName = (s: StudentRow) => {
    if ((s as any).academicYearName && (s as any).academicYearName !== "—") {
      return (s as any).academicYearName;
    }

    if ((s as any).academic_year?.display_name) {
      return (s as any).academic_year.display_name;
    }

    if ((s as any).academic_year?.name) {
      return (s as any).academic_year.name;
    }
    ...
    if (current?.academic_year?.display_name) {
      return current.academic_year.display_name;
    }

    if (current?.academic_year?.name) {
      return current.academic_year.name;
    }

    const yearId = resolveAcademicYearId(s);
    if (yearId && data?.academicYears?.length) {
      const match = data.academicYears.find((ay) => ay.id === yearId);
  ...
  const filtered = useMemo(() => {
    ...
    return data.students.filter((s) => {
      ...
      const matchY =
        academicYear === "all" || resolveAcademicYearId(s) === academicYear;
      const matchS = matchesStudentStatusFilter(s.status, status);
      return matchQ && matchF && matchY && matchS;
    });
  }, [data, query, faculty, academicYear, status]);
  ...
  const exportToExcel = async () => {
    ...
    worksheet.columns.forEach((col: Partial<import("exceljs").Column>) => {
      col.width = 20;
    });
  ...
  const columns: Column<StudentRow>[] = [
    ...
    {
      key: "phone",
      header: i18n.apparitorat.phone_label,
      render: (s) => s.phone_number || "—",
    },
    {
      key: "faculty",
      header: i18n.apparitorat.faculty,
      render: (s) => s.facultyCode,
    },
    {
      key: "promotion",
      header: i18n.apparitorat.promotion,
      render: (s) => s.promotionName,
    },
    {
      key: "academic_year",
      header: "Année Académique",
      render: (s) => resolveAcademicYearName(s),
    },
    {
      key: "status",
      header: "Statut",
    ...
    <>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <Select value={academicYear} onValueChange={setAcademicYear}>
          <SelectTrigger className="flex-1 sm:w-48 sm:flex-none">
            <SelectValue placeholder={i18n.common.academic_year} />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent>
            ...
            <SelectItem value="pending">
              {i18n.apparitorat.status_pending}
            </SelectItem>
            <SelectItem value="suspended">
              {i18n.apparitorat.status_suspended}
            </SelectItem>
            <SelectItem value="finished">
              {i18n.apparitorat.status_finished}
            </SelectItem>
            <SelectItem value="dropped">
              {i18n.apparitorat.status_dropped}
            </SelectItem>
