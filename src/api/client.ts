...
async function request<T>(
  ...
  if (!res.ok) {
    ...
    try {
      ...
      const eb = errorBody as Record<string, string>;
      errorMsg = eb.error || eb.message || eb.err || errorMsg;
    } catch {
      try {
        errorBody = await res.text();
        const bodyStr = errorBody as string;
        if (bodyStr && bodyStr.length < 200) errorMsg = bodyStr;
      } catch {
        errorBody = null;
      }
    }
