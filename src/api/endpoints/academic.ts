...
export const ENDPOINTS = {
  teachingUnits: {
    base: "/academics/teaching-units",
    detail: (id: string) => `/academics/teaching-units/${id}`,
  },
  faculties: {
  ...
  evaluations: {
    ...
    byHistory: (historyId: string) =>
      `/academics/evaluations/history/${historyId}`,
  },
  grades: {
    base: "/academics/grades",
    detail: (id: string) => `/academics/grades/${id}`,
    byEvaluation: (evaluationId: string) =>
      `/academics/grades/evaluations/${evaluationId}`,
...
export const teachingUnitApi = {
  list: (params?: Record<string, string>) => {
    const qs = params ? "?" + new URLSearchParams(params).toString() : "";
    return api.get<TeachingUnit[]>(`${ENDPOINTS.teachingUnits.base}${qs}`);
  },
  get: (id: string) =>
    api.get<TeachingUnit>(ENDPOINTS.teachingUnits.detail(id)),
  create: (body: {
    name: string;
    promotion_id: string;
    semester: 1 | 2;
    code?: string;
    credit?: number;
  }) => api.post<TeachingUnit>(ENDPOINTS.teachingUnits.base, body),
  update: (id: string, body: Partial<{ name: string; code: string; semester: 1 | 2; credit: number }>) =>
    api.put<TeachingUnit>(ENDPOINTS.teachingUnits.detail(id), body),
  delete: (id: string) => api.delete<void>(ENDPOINTS.teachingUnits.detail(id)),
};
...
export const facultyApi = {
  ...
  create: (payload: { name: string; code: string }) =>
    api.post<Faculty>(ENDPOINTS.faculties.base, payload),
  update: (id: string, payload: Partial<Faculty>) =>
    api.put<Faculty>(ENDPOINTS.faculties.detail(id), payload),
...
export const promotionApi = {
  ...
  create: (payload: { name: string; code: string; faculty_id: string }) =>
    api.post<Promotion>(ENDPOINTS.promotions.base, payload),
  update: (id: string, payload: Partial<Promotion>) =>
    api.put<Promotion>(ENDPOINTS.promotions.detail(id), payload),
...
export const courseApi = {
  ...
  assignTeacher: (courseId: string, teacherId: string) =>
    api.patch<Course>(ENDPOINTS.courses.detail(courseId), { teacher_id: teacherId }),
  ...
  update: (id: string, payload: Partial<Course>) =>
    api.put<Course>(ENDPOINTS.courses.detail(id), payload),
...
export const roomApi = {
  ...
  update: (id: string, payload: Partial<Room>) =>
    api.put<Room>(ENDPOINTS.rooms.detail(id), payload),
...
export const evaluationApi = {
  ...
  listByHistory: (historyId: string) =>
    api.get<Evaluation[]>(ENDPOINTS.evaluations.byHistory(historyId)),
};
...
export const gradeApi = {
  ...
  update: (id: string, payload: Partial<Grade>) =>
    api.put<Grade>(ENDPOINTS.grades.detail(id), payload),
  listByEvaluation: (evaluationId: string) =>
    api.get<Grade[]>(ENDPOINTS.grades.byEvaluation(evaluationId)),
  publish: (grade_ids: string[]) =>
    api.post<void>(ENDPOINTS.grades.publish, { grade_ids }),
  /** Upsert: create if no id, update if id present */
  upsert: (payload: { id?: string; evaluation_id?: string; student_id?: string; score_obtained?: number }) =>
    payload.id
      ? api.put<Grade>(ENDPOINTS.grades.detail(payload.id), payload)
      : api.post<Grade>(ENDPOINTS.grades.base, payload),
  /** Legacy status update (maps to partial update) */
  updateStatus: (id: string, status: string) =>
    api.put<Grade>(ENDPOINTS.grades.detail(id), { status }),
  getStudentAverage: (studentId: string, historyId?: string) => {
    const qs = historyId ? `?history_id=${historyId}` : "";
    return api.get<StudentAnnualAverage>(
      `${ENDPOINTS.averages.student(studentId)}${qs}`,
    );
  },
  getPromotionAverages: (promotionId: string, historyId?: string) => {
    const qs = historyId ? `?history_id=${historyId}` : "";
    return api.get<StudentAnnualAverage[]>(
      `${ENDPOINTS.averages.promotion(promotionId)}${qs}`,
    );
  },
};
...
export const academicYearApi = {
  ...
  create: (payload: {
    ...
    end_date: string;
    is_active?: boolean;
  }) => api.post<AcademicYear>(ENDPOINTS.academicYears.base, payload),
  activate: (id: string) =>
    api.post<void>(ENDPOINTS.academicYears.activate(id), {}),
};
...
export const appealApi = {
  ...
  create: (body: unknown) =>
    api.post<GradeAppeal>(ENDPOINTS.appeals.base, body),
...
export const assignmentApi = {
  ...
  create: (body: unknown) =>
    api.post<Assignment>(ENDPOINTS.assignments.base, body),
...
export const submissionApi = {
  ...
  create: (body: unknown) =>
    api.post<Submission>(ENDPOINTS.submissions.base, body),
...
export const resourceApi = {
  ...
  create: (body: unknown) =>
    api.post<CourseResource>(ENDPOINTS.resources.base, body),
